...
export function TransferHistory({ children }: TransferHistoryProps) {
    ...
    if (dateFilter !== 'all') {
      ...
      switch (dateFilter) {
        case 'today':
          ...
          break;
        case 'week':
          filtered = filtered.filter(record => now - record.timestamp < 7 * dayMs);
          break;
        case 'month':
          filtered = filtered.filter(record => now - record.timestamp < 30 * dayMs);
          break;
      }
    }
  ...
  const getStatistics = () => {
    ...
    const receivedFiles = completed.filter(r => r.direction === 'received').length;
